 ...
 * @returns {Stream}
 */
GFSupload.prototype.getFileStream = function(id) {
  return gfsStream.createReadStream({_id: id});
}
...
GFSupload.prototype._putFile = function(path, name, options, fn) {
  ...
  return textract(options.content_type, path, extractOptions, function( error, text ) {
    ...
    if(options.content_type == 'image/jpeg') {
      return new ExifImage({ image : path }, function (exifError, exifData) {
        if(exifData) {
 ...
 * @deprecated Using remove from gridfs-stream
 */
GFSupload.prototype.deleteFile = function(id, options, callback) {
  ...
  options.root = this.prefix;
  return new self.GridStore(db, id, 'r', options).open(function(err, file) {
    if(err) {
